...
%% basic settings
fileNumStart=1001; % This is not used in a loop, but kept for context
fileNumEnd=10000;
fileNumInterval=1;
...
threshold_magnitude = 0.05; % <--- Adjust the threshold magnitude here

% Create a binary mask for positive (counter-clockwise) vortices
binary_mask_pos = enstrophy_buoyancy > threshold_magnitude;

% Create a binary mask for negative (clockwise) vortices
binary_mask_neg = enstrophy_buoyancy < -threshold_magnitude;
...
if num_vortices_pos > 0
    ...
    disp('Positive vortices sorted and relabeled by size.');
        ...
        % *** MODIFICATION START: Filter vortices to find the largest in the central region ***
    central_vortex_indices = [];
    central_vortex_areas = [];
    for i = 1:num_vortices_pos
        centroid = stats_pos(i).Centroid;
        % Check if the centroid is within the 0.3-0.7 bounding box
        if centroid(1) > 0.4 && centroid(1) < 0.6 && centroid(2) > 0.4 && centroid(2) < 0.6
            central_vortex_indices(end+1) = i;
            central_vortex_areas(end+1) = stats_pos(i).Area;
        end
    end

    if ~isempty(central_vortex_indices)
        % Find the largest area AMONG THE CENTRAL VORTICES
        [~, max_area_idx_in_filtered_list] = max(central_vortex_areas);
        % Get the original label of this largest central vortex
        largest_central_vortex_label = central_vortex_indices(max_area_idx_in_filtered_list);
        % Extract its centroid
        lsc_center_pos = stats_pos(largest_central_vortex_label).Centroid;
        disp(['Found ', num2str(length(central_vortex_indices)), ' vortices in the central region.']);
        disp(['Largest CENTRAL positive vortex center at (X, Y) = (', ...
              num2str(lsc_center_pos(1)), ', ', num2str(lsc_center_pos(2)), ')']);
    else
        disp('No positive vortices found within the central region (0.3<x<0.7, 0.3<y<0.7).');
    end
    % *** MODIFICATION END ***
% 
...
if num_vortices_neg > 0
    ...
    central_vortex_areas = [];
    for i = 1:num_vortices_neg
        centroid = stats_neg(i).Centroid;
        if centroid(1) > 0.4 && centroid(1) < 0.6 && centroid(2) > 0.4 && centroid(2) < 0.6
            central_vortex_indices(end+1) = i;
            central_vortex_areas(end+1) = stats_neg(i).Area;
        end
    end
    ...
    if ~isempty(central_vortex_indices)
        [~, max_area_idx_in_filtered_list] = max(central_vortex_areas);
        largest_central_vortex_label = central_vortex_indices(max_area_idx_in_filtered_list);
        lsc_center_neg = stats_neg(largest_central_vortex_label).Centroid;
        disp(['Found ', num2str(length(central_vortex_indices)), ' vortices in the central region.']);
        disp(['Largest CENTRAL negative vortex center at (X, Y) = (', ...
              num2str(lsc_center_neg(1)), ', ', num2str(lsc_center_neg(2)), ')']);
    else
        disp('No negative vortices found within the central region (0.3<x<0.7, 0.3<y<0.7).');
    end
    % *** MODIFICATION END ***
...
tec_file.FileName = 'enstrophy_buoyancy';
tec_file.Variables = {'x','y','enstrophy_buoyancy'};
tec_file.Zones = liton_ordered_tec.TEC_ZONE;
tec_file.Zones.Data = {Cx,Cy,enstrophy_buoyancy};
tec_file.write_plt();
...
if num_vortices_pos > 0
    ...
    tec_file = liton_ordered_tec.TEC_FILE;
    tec_file.FileName = 'enstrophy_buoyancy_inden_pos';
    tec_file.Variables = {'x','y','enstrophy_buoyancy_inden_pos'};
    tec_file.Zones = liton_ordered_tec.TEC_ZONE;
    ...
    tec_file = liton_ordered_tec.TEC_FILE;
    tec_file.FileName = 'enstroBuoyancy_sorted_labels_pos';
...
if num_vortices_neg > 0
    ...
    tec_file = liton_ordered_tec.TEC_FILE;
    tec_file.FileName = 'enstrophy_buoyancy_inden_neg';
    tec_file.Variables = {'x','y','enstrophy_buoyancy_inden_neg'};
    tec_file.Zones = liton_ordered_tec.TEC_ZONE;
    ...
    tec_file = liton_ordered_tec.TEC_FILE;
    tec_file.FileName = 'enstroBuoyancy_sorted_labels_neg';
...
disp(' ');
disp('--- Generating and Saving Plots ---');

% --- Visualization for Positive Vortices ---
if num_vortices_pos > 0
    % 1. Create a figure but keep it invisible
    fig_pos = figure('Visible', 'off'); 
    
    % All plotting commands will operate on the invisible figure
    custom_colormap_pos = [1 1 1; jet(num_vortices_pos)];
    % IMPORTANT: Transpose the mask to align data with axes
    contourf(Cx, Cy, relabeled_mask_pos, 0:num_vortices_pos, 'LineStyle', 'none');
    
    hold on;
    % Only plot the star if a central vortex was found
    if ~isnan(lsc_center_pos(1))
        plot(lsc_center_pos(2), lsc_center_pos(1), 'p', ...
    ...
    c = colorbar('Ticks', (1:num_vortices_pos) - 0.5, 'TickLabels', 1:num_vortices_pos);
    c.Label.String = 'Vortex Label (Sorted by Size)';
    title({'Sorted Positive (CCW) Vortices', 'with Largest Central Vortex Marked'});
    xlabel('x'); ylabel('y');
    
    % 2. Define the output filename
    output_filename_pos = 'sorted_positive_vortices.png';
    
    % 3. Save the invisible figure directly to a file
    saveas(fig_pos, output_filename_pos);
    
    % 4. Close the figure to free up memory (very important!)
    close(fig_pos);
    disp(['Saved image: ', output_filename_pos]);
end

% --- Visualization for Negative Vortices ---
if num_vortices_neg > 0
    % 1. Create an invisible figure
    fig_neg = figure('Visible', 'off');
    ...
    hold on;
    % Only plot the star if a central vortex was found
    if ~isnan(lsc_center_neg(1))
        plot(lsc_center_neg(2), lsc_center_neg(1), 'p', ...
            'MarkerEdgeColor','k', 'MarkerFaceColor','y', 'MarkerSize',15, 'LineWidth', 1.5);
    end
    hold off;
    ...
    c = colorbar('Ticks', (1:num_vortices_neg) - 0.5, 'TickLabels', 1:num_vortices_neg);
    c.Label.String = 'Vortex Label (Sorted by Size)';
    title({'Sorted Negative (CW) Vortices', 'with Largest Central Vortex Marked'});
    xlabel('x'); ylabel('y');
    
    % 2. Define the output filename
    output_filename_neg = 'sorted_negative_vortices.png';
    
    % 3. Save the figure to a file
    saveas(fig_neg, output_filename_neg);
    
    % 4. Close the figure
    close(fig_neg);
    disp(['Saved image: ', output_filename_neg]);
end
%%
